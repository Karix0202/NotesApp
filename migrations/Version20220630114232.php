 ...
 */
final class Version20220630114232 extends AbstractMigration
{
    ...
    public function up(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__note AS SELECT id, title, content, created_at, color FROM note');
        ...
        $this->addSql('INSERT INTO note (id, title, content, created_at, color) SELECT id, title, content, created_at, color FROM __temp__note');
    ...
    public function down(Schema $schema): void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__note AS SELECT id, title, content, created_at, color FROM note');
        $this->addSql('DROP TABLE note');
        $this->addSql('CREATE TABLE note (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, title VARCHAR(255) NOT NULL, content CLOB NOT NULL, created_at DATETIME NOT NULL, color CLOB NOT NULL)');
        $this->addSql('INSERT INTO note (id, title, content, created_at, color) SELECT id, title, content, created_at, color FROM __temp__note');
        $this->addSql('DROP TABLE __temp__note');
    }
}
